...
def main():
    parser = argparse.ArgumentParser(description="Append a row to a Google Sheet worksheet.")
    parser.add_argument("--url", required=True, help="Spreadsheet URL")
    parser.add_argument("--values", required=True, help="Comma-separated cell values")
    ...
    try:
        ...
        print(json.dumps({"status": "appended", "worksheet": ws.title, "values": row}))
