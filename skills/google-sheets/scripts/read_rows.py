#!/usr/bin/env python3
# /// script
# requires-python = ">=3.10"
# dependencies = [
#   "gspread>=6.0",
#   "google-auth>=2.0",
# ]
# ///
"""Read all rows from a worksheet as a list of dicts."""
import argparse
import json
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent))
from src import get_worksheet


def main():
    parser = argparse.ArgumentParser(description="Read all rows from a Google Sheet worksheet.")
    parser.add_argument("--url", required=True, help="Spreadsheet URL")
    parser.add_argument("--worksheet", type=int, default=0, help="Worksheet index (default: 0)")
    parser.add_argument("--credentials", default=None, help="Path to credentials.json")
    args = parser.parse_args()

    try:
        ws = get_worksheet(args.url, args.worksheet, args.credentials)
        records = ws.get_all_records()
        print(json.dumps({"status": "ok", "worksheet": ws.title, "count": len(records), "rows": records}))
    except FileNotFoundError as e:
        print(json.dumps({"status": "error", "error": str(e)}), file=sys.stderr)
        sys.exit(1)
    except Exception as e:
        print(json.dumps({"status": "error", "error": str(e)}), file=sys.stderr)
        sys.exit(1)


if __name__ == "__main__":
    main()
